...
func eventAuthFailed(r uniform.IRequest, p diary.IPage) {
	...
	db.CatchNoDocumentsErr(func(p diary.IPage) {
		...
		case "administrator":
			db.Read(r.Remainder(), Database, CollectionAdministrators, request.Id, &response, nil)
			if response.LockedAt == nil && response.BlockedAt == nil {
				if response.Counter >= 2 {
					db.Update(r.Remainder(), Database, CollectionAdministrators, request.Id, uniform.M{
						"lockedAt": time.Now(),
					}, nil, nil)
				} else {
					db.Inc(r.Remainder(), Database, CollectionAdministrators, request.Id, "counter", 1, nil, nil)
				}
			}
			break
		}
